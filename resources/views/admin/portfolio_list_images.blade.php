                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <a href="javascript:void(0)" data-toggle="modal" data-target="#exampleModal"
                               class="btn btn-primary btn-block">Portfolio'ya Yeni Resim Ekle</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <h4 class="card-title">Portfolio Resmi</h4>
                            ...
                            @foreach($images as $item)
                                <tr>
                                    ...
                                    <td><a data-id="{{ $item->id }}" href="javascript:void(0)"
                                           class="btn btn-danger deleteImage"> Sil <i class="fa fa-trash"></i></a>
                                    </td>
                                    <td><a data-id="{{ $item->id }}" href="javascript:void(0)"
                                           class="btn
...
@section('js')
    <script>
        ...
        $('.changeStatus').click(function ()
        {
            ...
            let finalRoute = route.replace('featureImage', imageID);
            $.ajax({
                url: finalRoute,
                type: "POST",
                async: false,
        ...
        $('.deleteImage').click(function ()
        {
            ...
            Swal.fire({
                title: "Emin misiniz?",
                text: imageID + " ID'li portfolio resmini silmek istediğinize emin misiniz? ",
                icon: 'warning',
            ...
            }).then((result) =>
            {
                if (result.isConfirmed)
                {
                    ...
                    let finalRoute = route.replace('deletePortfolio', imageID);
                    $.ajax({
                        url: finalRoute,
                        type: "POST",
                        async: false,
